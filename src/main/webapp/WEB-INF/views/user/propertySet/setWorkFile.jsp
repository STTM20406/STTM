...
<%@taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
    ...
    <form id="frm1" role="form" action="/main/insertFilePost" method="post" enctype="multipart/form-data">
	...
	<c:choose>
		...
		<c:otherwise>
			<li><a
				href="/main/filePagingList?wrk_id=1&page=${pageVo.page-1}&pageSize=${pageVo.pageSize}">&lt; </a>
			</li>
		</c:otherwise>
	</c:choose>
	...
	<c:forEach var="i" begin="1" end="${paginationSize}">
		<c:choose>
			...
			<c:otherwise>
				<li><a
					href="/main/filePagingList?wrk_id=1&page=${i}&pageSize=${pageVo.pageSize}">${i}</a>
				</li>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<!-- link -->
	<form id="frm2" role="form" action="/main/insertLinkPost" method="post" >
		<input type="text" name="attch_url"/>
		<button type="submit">등록</button>
	</form>
    <hr>	
	<div>
	...
	<c:choose>
		...
		<c:otherwise>
			<li><a
				href="/main/linkPagingList?wrk_id=1&page=${pageVo.page-1}&pageSize=${pageVo.pageSize}">&lt; </a>
			</li>
		</c:otherwise>
	</c:choose>
	...
	<c:forEach var="i" begin="1" end="${paginationSize}">
		<c:choose>
			...
			<c:otherwise>
				<li><a
					href="/main/linkPagingList?wrk_id=1&page=${i}&pageSize=${pageVo.pageSize}">${i}</a>
				</li>
			</c:otherwise>
		</c:choose>
	</c:forEach>
